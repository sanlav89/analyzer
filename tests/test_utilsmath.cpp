...
TEST(MatlabUtils, calcEnergy)
{
    ...
    auto energy = math::calcEnergy(energyPoly);
...
TEST(MatlabUtils, calcFwhm)
{
    ...
    auto energy = math::calcEnergy(energyPoly);
    auto fwhm = math::calcFwhm(energy[99], fwhmPoly);
...
TEST(MatlabUtils, calcEffifiency)
{
    ...
    auto efficiency = math::calcEffifiency(500.0, efficiencyPoly);
...
TEST(MatlabUtils, gaussian)
{
    ...
    auto x = math::calcEnergy(energyPoly);
    auto sigma = 20;
    auto x0 = 500;
    auto area = 100;

    auto gaussian = math::gaussian(x, x0, sigma, area);
...
TEST(MatlabUtils, substrateModel)
{
    ...
    auto substrate = math::substrateModel(x, a, b, c, d);
...
TEST(MatlabUtils, generatePortion)
{
    ...
    auto portion = math::generatePortion(spectrum, 100);
